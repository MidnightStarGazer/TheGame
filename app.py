...
# --- VAST PLAINS ---
    ...
    st.write("You found yourself in the middle of a vast grassy field. You feel the wind blowing gently on your face...")

    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("Search the rocks"):
            with st.spinner("Searching the rocks..."): time.sleep(2) 
    ...
    with col2:
        if st.button("Follow the dirt track"):
            with st.spinner("Walking toward the village..."): time.sleep(1.5)
            st.session_state.location = "Village"
            st.rerun()
    with col3:
        if st.button("Walk south"):
            with st.spinner("Walking towards the lake in the distance..."): time.sleep(2)
